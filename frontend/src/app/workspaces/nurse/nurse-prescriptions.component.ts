      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-4 border-b border-border">
        <div>
          <h1 class="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
            Medication MAR Orders
            <span hlmBadge variant="secondary" class="text-[10px]">Nursing Administration</span>
          </h1>
          <p class="text-xs text-muted-foreground mt-0.5">Read-only verification of active physician eRx orders for administration.</p>
        </div>
      </div>
          ...
          <table hlmTable class="w-full text-xs">
            ...
            <tbody hlmTableBody class="divide-y divide-border">
              ...
              <tr *ngIf="prescriptions().length === 0" hlmTableRow>
                <td colspan="5" hlmTableCell class="py-12 text-center text-muted-foreground text-xs">No active eRx orders logged.</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export class NursePrescriptionsComponent implements OnInit {
  ...
  constructor(
    ...
    effect(() => {
      ...
      if (active) this.loadRx(active.id);
    });
  }
